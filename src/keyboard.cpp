...
static void keyboard_run_key_sequence(button_sequence_t sequence, keyboard_button_keyboard_mode_t mode)
{
  ...
  for (uint8_t i = 0; i < sequence.length; i++)
  {
    ...
    delay(50);  // hold key long enough for KVM to detect
    if (sequence.delay > 0)
    {
...
static void keyboard_run_mouse_sequence(button_mouse_t sequence, keyboard_button_keyboard_mode_t mode)
{
  ...
  for (uint8_t i = 0; i < sequence.length; i++)
  {
    ...
    case SCROLL_DOWN:
      ...
      break;
    case VOLUME_UP:
      Consumer_send(CONSUMER_VOLUME_UP);
      delay(20);
      Consumer_send(0);  // Release
      break;
    case VOLUME_DOWN:
      Consumer_send(CONSUMER_VOLUME_DOWN);
      delay(20);
      Consumer_send(0);  // Release
      break;
    case MUTE_TOGGLE:
      Consumer_send(CONSUMER_MUTE);
      delay(20);
      Consumer_send(0);  // Release
      break;
    default:
